...
<template>
  <div>
    <div id="cardTop"></div>
    <div class="grid card rounded-box place-items-center" v-for="item in blogList">
      <div class="card lg:card-side bg-base-100 shadow-xl lg:max-w-4xl">
        <figure class="lg:w-1/2"><img :src="item.cover" alt="Album"></figure>
        <div class="card-body lg:w-1/2">
          <h2 class="card-title"><a :href="'/article/' + item.id">{{ item.title }}</a></h2>
          <div class="text-xs">
    ...
    <div class="grid place-items-center pb-8">
      <div class="btn-group grid grid-cols-2 lg:max-w-4xl">
        <button class="btn btn-outline bg-base-100 border-none" @click="toNext(0)">Previous page</button>
        <button class="btn btn-outline bg-base-100 border-none" @click="toNext(1)">Next</button>
      </div>
    </div>
  </div>
</template>